...
try {
   ...
   // $printer1->text(new item('', '$'));
	...
	{
	   return new item($item["name"],$item["count"],$item["price"],array_key_exists('extras' , $item) ? $item["extras"] : [],array_key_exists('slots' , $item) ? $item["slots"] : [],true);
	}
	...
	$printer1->feed();
    $printer1->setTextSize(2,2);
    $printer1->text("Avoir sur titre \n");
    $printer1->selectPrintMode();
	$printer1->text("Valable jusqu'au \n");	
	$printer1->text($expires_at."\n");	
	$printer1->feed();
    $printer1->feed();
    $printer1->setTextSize(2,2);
    $printer1->text($discount."EUR\n");
    $printer1->selectPrintMode();
	...
	// 	// var_dump($elem) ;
	// 		$d=array_sum($tvas[$key]);
	// 	$printer1->text("TVA(".$key."%)    :     ".$d."€\n");
	// 	$tvanew=$tvanew+$d;

	// }
	
	//     $ede=$HTOut+$tvanew;
	// 	$printer1->text("TTC         :     ".$ede."€\n");
	// 	if($payments){
